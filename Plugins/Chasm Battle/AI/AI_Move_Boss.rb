class PokeBattle_AI
        ...
        if choices.length > 0
            ...
            if guaranteedChoices.length == 0
                if empoweredDamagingChoices.length > 0
                    ...
                    sortedChoices = regularChoices.stable_sort_by { |choice| -choice[1] }
    ...
    def bossMovesChosen(user, choice, bossAI)
        ...
        if move.damagingMove?(true) && move.empoweredMove?
            @battle.pbDisplayBossNarration(_INTL("{1} is winding up a big attack!", user.pbThis)) if $Options.avatar_mechanics_messages == 0
